--drop table analytics.online_roster_v2 
--Activity Online 

create  table analytics.activity_online as (select main.student_id,log_date,max(Case when event_id= 38 then 1 else 0 end ) as live_class_clicked  from allen_digital_reporting.activity_log main 
join prod.student a on a.student_id= main.student_id 
and a.center_name = 'ALLEN DIGITAL'
group by 1,2)


create table analytics.online_roster_v2 as 
...
join
  ...
  left join analytics.activity_online as activity_online on activity_online.student_id = asa.student_id and activity_online.log_date = date(main.class_Date_time) 
